...
exports.up = async(function (db) {
  ...
  const users = awaitFor(db._find('users', {picture: {$ne: null}}))
  users.forEach(u => awaitFor(usersCollection.update({_id: u._id}, {$set: {s3Key: u.legajo}}, {multi: true})))
  awaitFor(usersCollection.update({}, {$unset: {picture: 1}}, {multi: true}))
  return true
})
