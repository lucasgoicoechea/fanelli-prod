  ...
  <div class="form-list">
    ...
    <div class="container-fluid" v-show="!loading">
      <div class="col-xs-6 col-sm-4 col-md-2"  v-for="supervisionpartDate in getDatesSupervisionparts">
        <spinner :show="loading"></spinner>
        <supervisionpart-resume-date-button :key="supervisionpartDate" :date="supervisionpartDate">
        </supervisionpart-resume-date-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  import SupervisionpartResumeDateButton from '@/components/supervisionPartsL3/SupervisionpartResumeDateButton.vue'
    ...
    methods: {
      redirect: function () {
        if (this.date !== '') {
          ...
          this.$router.push({name: 'supervisionpartsResumen-linea3', params: {date: formatedDate}})
        } else {
