<template>
  <div class="Supervisionpart-history">
    ...
    <div v-else class="container-fluid">
      <header>
        <div class="day">
          <router-link :to="{ name: 'supervisionpartsHistory-linea3', params: { date: prevDate}}">
            {{ prevDate | formatDate }}
          </router-link>
        </div>
        <div class="current">{{ date | formatDate }}</div>
        <div class="day">
          <router-link :to="{ name: 'supervisionpartsHistory-linea3', params: { date: nextDate}}">
            {{ nextDate | formatDate }}
          </router-link>
        </div>
      </header>
...
<script>
  ...
  export default {
    name: 'SupervisionpartHistory',
    components: {
      Navigation,
      Tabs,
      Tab,
      summaryView: SupervisionpartSummaryView,
