<template>
  <article class="supervisionpart-history-stoppings">
    <header>
       <h3> </h3>
      <div  style="margin-right: 20px;" >Paradas</div>
    </header>
     
  <section >
 <section  style=" background-color: #F0AC83;" >
      <div v-if="!stoppings || stoppings.length == 0">
          <span>No hay paradas de la hora.</span>
      </div>
      <div  v-for="o in stoppings" :key="o._id">
...
<script>
  ...
  export default {
    name: 'SupervisionpartHistoryStoppings',
    props: {
      stoppings: Array,
      date: {
        type: String,
