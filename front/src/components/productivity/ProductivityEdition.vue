<template>
  <div class="productivity-edition">
    ...
    <bottom-navbar>
      <div class="submit">
        <div
          ...
          class="msg pointer"
          @click="confirm">
          <h5 class="items">Editar desempeño</h5>
          <span class="glyphicon glyphicon-send items icon-action"></span>
        </div>

        <div class="msg" v-else>
          <h5 class="items">Editando desempeño</h5>
          <spinner
            class="items"
...
<script>
  ...
  import FormComponent from '@/components/productivity/ProductivityForm'

  export default {
    name: 'productivityEdition',
    components: {
      BottomNavbar,
      Navigation,
    ...
    data () {
      return {
        ...
        validation: {},
        loading: false,
        fetchStoreAction: 'productivitys/getDetail',
        editStoreAction: 'productivitys/edit'
      }
    },
    created () {
      this.getDetails()
    ...
    methods: {
      getDetails () {
        return this.$store.dispatch(this.fetchStoreAction, {
          ...
          .then((res) => {
            this.fetched = res.productivity
            this.form = res.productivity
          })
          .catch(() => { this.$snotifyWrapper.error('Se produjo un error al obtener la desempeño') })
      },
      update (data) {
        this.form = data.form
      ...
      successfulEdition (res) {
        this.loading = false
        if (res.success) {
          this.$snotifyWrapper.success('La desempeño se ha editado con exito')
          this.$router.push({name: 'productivity-detail', params: {id: this.form._id}})
        } else {
      ...
      confirm () {
        ...
        this.$modal.show('dialog', {
          title: 'Confirmación',
          text: '¿Esta seguro de actualizar esta desempeño?',
          buttons: [
            {
              title: 'Aceptar',
              handler: this.edit
