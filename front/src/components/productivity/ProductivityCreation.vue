<template>
  <div class="productivity-creation">
    <navigation
    ...
    <bottom-navbar>
      <div class="submit">
        <div
          ...
          class="msg pointer"
          @click="confirm">
          <h5 class="items">Crear desempeño</h5>
          <span class="glyphicon glyphicon-send items icon-action"></span>
        </div>

        <div class="msg" v-else>
          <h5 class="items">Creando desempeño</h5>
          <spinner
            class="items"
  ...
  export default {
    ...
    data () {
      return {
        ...
        validation: {},
        loading: false,
        sendStoreAction: 'productivitys/create'
      }
    },
    methods: {
      update (data) {
      ...
      successfulCreation (res) {
        this.loading = false
        if (res.success) {
          this.$snotifyWrapper.success('La desempeño se ha creado con exito')
      ...
      confirm () {
        ...
        this.$modal.show('dialog', {
          title: 'Confirmación',
          text: '¿Esta seguro de generar esta desempeño?',
          buttons: [
            {
              title: 'Aceptar',
              handler: () => {
