...
const supervisionpartLTresSchema = new Schema({
  supervisorShift: {
    type: Schema.Types.ObjectId,
    ref: 'User'
  },
...
supervisionpartLTresSchema.statics.getLastSupervisionPartOfSector = function (sector) {
...
supervisionpartLTresSchema.statics.getSupervisionPartForDayAndSector = function (date, sector) {
...
supervisionpartLTresSchema.statics.getSupervisionPartForDay = function (date) {
  // we can't search by date. We must query between beginning of the day and the end of the day
  const beginning = new Date(date.getTime())
  // Adding 24 hs
  const ending = new Date(beginning.getTime() + (24 * 60 * 60 * 1000))
