...
const SupervisionpartModel = require(path.join(__dirname, '/../model')).supervisionPartLTres
...
const controller = {
  add: async(function (req, res, next) {
    ...
    if (supervisionpartTmp.supervisor === undefined) {
      ...
      // //winston.log('info', 'Primer valor de la supervision', supervisionpartTmp.sector)
  ...
  summary: async(function (req, res) {
    ...
    // //winston.log('info', 'date it s:', {date: date, localDate: date.toLocaleString()})
    SECTORS.forEach(s => {
  ...
  comparative: async(function (req, res, next) {
    ...
    // //winston.log('info', 'supervisionpart date it s:', {date: date, localDate: date.toLocaleString()})
  ...
  totales: async(function (req, res, next) {
    ...
    // //winston.log('info', 'vengo por los totales fecha:', {date: date, localDate: date.toLocaleString()})
  ...
  totalesExtrusora: async(function (req, res, next) {
    ...
    // //winston.log('info', 'supervisionpart totalesExtrusora:', {date: date, localDate: date.toLocaleString()})
  ...
  totalesApiladora: async(function (req, res, next) {
    ...
    // //winston.log('info', 'supervisionpart totalesApiladora:', {date: date, localDate: date.toLocaleString()})
  ...
  totalesDesapiladora: async(function (req, res, next) {
    ...
    // //winston.log('info', 'supervisionpart totalesDesapiladora :', {date: date, localDate: date.toLocaleString()})
    const sector =  'DESAPILADORA'   
    let supervisionparts = awaitFor(SupervisionpartModel.getSupervisionPartForDayAndSector(date, sector))
    //ARMO ARREGLO CLAVE=MATEIRAL Y VALOR=TONELADAS PALLETS
    let totalesMaterial = new Array()
...
const updateReminders = async(function (hours) {
  ...
  if (someCheckIsFalse && !awaitFor(notification.notificationExists('something-bad'))) {
    // //winston.log('info', 'Hay algo que corregir y no hay reminder')
    notification.pushNotificationEvery(
      config.CHECKLIST_REMINDER,
      {user_type: USER_TYPE.SUPERVISOR_PRODUCCION},
    ...
    // //winston.log('info', 'Todo esta bien')
    notification.clearNotification('something-bad')
  }
  ...
  if (missingChecks && !awaitFor(notification.notificationExists('must-complete'))) {
    // //winston.log('info', 'Faltan campos que completar del parte de supervision y no hay un reminder')
    notification.pushNotificationEvery(
      config.CHECKLIST_REMINDER,
      {user_type: USER_TYPE.SUPERVISOR_PRODUCCION},
    ...
    // //winston.log('info', 'Todo el parte de supervision esta completo')
    notification.clearNotification('must-complete')
  }
})
